...
%Inicializacion de la mixtura basado en la inicializacion de parametros
for c=ll'; 
  ic=find(c==ll); 
  idc=find(xl==c); 
  Nc=rows(idc);
  ...
  S(ic,1:K)=a*cov(X(idc,:),1)/K+(1-a)*eye(D);
end
...
do; 
  ...
  for c=ll'; 
    ...
    z=[]; 
    for k=1:K; 
      z(:,k)=compute_zk(ic,k,pkGc,mu,S,X); 
    end
    mz=max(z,[],2); z=exp(z-mz); sz=sum(z,2); gtr(:,ic)=mz+log(sz);
    z=[]; 
    for k=1:K; 
      z(:,k)=compute_zk(ic,k,pkGc,mu,S,Y); 
    end
    mz=max(z,[],2); z=exp(z-mz); sz=sum(z,2); gte(:,ic)=mz+log(sz); 
  end
