...
def knapsack_forward(W, v, w):
    ...
    """
    Vcurr = {0: 0.}
    n_states = 0  # CAMBIAR Y COMPLETAR DONDE CORRESPONDA
...
def sparse_knapsack(W, v, w):
    ...
    col = [(0, 0)]
    n_states = 0  # CAMBIAR ESTO Y COMPLETAR DONDE CORRESPONDA

    for vi, wi in zip(v, w):
        # cada iteraciÃ³n es una etapa o columna de PD
        n_states += 1
        col = merge(col, [(weight+wi, value+vi)
                          for (weight, value) in col if weight+wi <= W])
...
def measure_knapsack_functions():
    ...
    Reportar, por salida estandar, para cada talla:

     - El tiempo medio de ejecucion de cada algoritmo.
     - El porcenaje de estados activos visitados respecto de la version PD hacia atras.
    ...
    for function, functname in functions:
        # asocia a cada talla una lista de (tiempo, estados)
        measures[functname] = {}
