...
def langford_directo(N, allsolutions):
    ...
    def is_promising(num, n):
        return (n+num+1 < len(seq) and (seq[n] == 0 and seq[n+num+1] == 0))

    def backtracking(num):
        if num <= 0:
            yield "-".join(map(str, seq))
        else:
	    # COMPLETAR Ramificar y llamar recursivamente a los hijos == que en las 8 reinas
        # num = 4
        # 1º 4 0 0 0 0 4 0 0
            for n in range(len(seq)):
                if is_promising(num, n):
                    seq[n]=num
                    seq[n+num+1]=num
