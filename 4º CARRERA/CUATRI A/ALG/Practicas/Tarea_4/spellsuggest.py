...
import re
import time
from distance import dp_levenshtein_backwards as levenshtein
from distance import dp_restricted_damerau_backwards as restricted
from distance import dp_intermediate_damerau_backwards as intermediate
...
class SpellSuggester:
    ...
    def suggest(self, term, distance="levenshtein_forward", threshold=1):
        ...
        assert distance in ["levenshtein","levenshtein_forward", "restricted", "intermediate"]

        if distance == "levenshtein":
            f = levenshtein
        if distance == "levenshtein_forward":
            f = levenshtein
...
class TrieSpellSuggester(SpellSuggester):
    ...
    def __init__(self, vocab_file_path):
        ...
        def suggest(self, term, distance="levenshtein", threshold=1):
            results = {} # diccionario termino:distancia
            t = self.trie
    ...
    ## Opcional
    # Levenshtein hacia delante TODO

    def dp_levenshtein_forwards(self, term, distance="levenshtein_forwards", threshold=1):
        ...
        for child_st in  t.iter_children(parent_st):
            active_states = {}
            for j in range(1, lterm):
                active_states[child_st] = min(D[child_st,j] + 1,
                                                D[child_st, j-1] + 1,
                                                D[child_st,j-1] + (t.get_label(child_st) != term[j-1]))
                if(min(D[:, j]) > threshold):
                        return threshold + 1    
        for i in range(1,states):
                if t.is_final(i) and D[i,lterm-1] <= threshold:
                    active_states[t.get_output(i)] = D[i,lterm-1]
